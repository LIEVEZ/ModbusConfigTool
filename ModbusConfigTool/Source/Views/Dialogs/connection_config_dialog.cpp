...
ConnectionConfigDialog::ConnectionConfigDialog(const ConnectionPort &port, QWidget *parent)
    : QDialog(parent), m_originalId(port.id)
{
    ...
    m_portName->setPlaceholderText(QStringLiteral("例如：主站1"));
    ...
    m_slaveAddress = new QSpinBox(this); m_slaveAddress->setRange(1, 247);
    common->addRow(QStringLiteral("端口名称"), m_portName);
    common->addRow(QStringLiteral("连接类型"), m_type); common->addRow(QStringLiteral("从站地址"), m_slaveAddress);
    ...
    for (const QSerialPortInfo &info : QSerialPortInfo::availablePorts()) { m_serialPort->addItem(info.portName()); }
    ...
    m_host->setText(port.profile.tcpHost); m_port->setValue(port.profile.tcpPort); m_pollInterval->setValue(port.profile.pollIntervalMs);
    m_slaveAddress->setValue(port.profile.slaveAddress);
    m_serialPort->setCurrentText(port.profile.serialPort); m_baudRate->setCurrentText(QString::number(port.profile.baudRate)); m_parity->setCurrentText(QString(port.profile.parity));
...
ConnectionPort ConnectionConfigDialog::port() const
{
    ...
    output.profile.connectionType = m_type->currentIndex() == 0 ? ConnectionType::Tcp : ConnectionType::Rtu;
    output.profile.tcpHost = m_host->text().trimmed(); output.profile.tcpPort = quint16(m_port->value());
    output.profile.serialPort = m_serialPort->currentText(); output.profile.baudRate = m_baudRate->currentText().toInt();
    output.profile.parity = m_parity->currentText().at(0); output.profile.pollIntervalMs = m_pollInterval->value();
    output.profile.slaveAddress = quint8(m_slaveAddress->value());
    return output;
}
